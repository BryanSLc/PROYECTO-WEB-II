...
// NOTA: el tipo se sigue llamando SQLiteStorage para no romper las referencias
// que ya existen en handlers.go y otros archivos. Internamente ahora usa
// Postgres. Si prefieres renombrarlo a "Storage" o "PostgresStorage", hazlo
// con un find & replace en todo el proyecto (Go no tiene problema, es solo texto).
type SQLiteStorage struct {
	db *gorm.DB
}

// NuevoPostgresStorage inicializa la conexión a Postgres, crea las tablas y aplica migraciones.
// dsn tiene el formato:
// "host=HOST user=USER password=PASSWORD dbname=DBNAME port=PORT sslmode=SSLMODE"
func NuevoPostgresStorage(dsn string) *SQLiteStorage {
	var db *gorm.DB
	var err error

	// Reintentos de conexión: en docker-compose la app puede arrancar
	// antes de que Postgres esté completamente listo para aceptar conexiones.
	maxIntentos := 10
...
// ConstruirDSN arma el string de conexión a partir de variables de entorno.

...
// obtenerEnv devuelve el valor de la variable de entorno o un valor por defecto si no existe.
...
func main() {
	// 1. Construimos el DSN de conexión a Postgres a partir de variables de entorno.
	//    Estas variables coinciden con las definidas en docker-compose.yml / .env
	dsn := storage.ConstruirDSN(
		obtenerEnv("DB_HOST", "localhost"),
		obtenerEnv("DB_PORT", "5432"),
		obtenerEnv("DB_USER", "archibase"),
		obtenerEnv("DB_PASSWORD", "archibase"),
		obtenerEnv("DB_NAME", "archibase"),
		obtenerEnv("DB_SSLMODE", "disable"),
	)

	// 2. Inicializamos el almacenamiento en Postgres
	almacen := storage.NuevoPostgresStorage(dsn)

	// 3. Inyectamos el almacén persistente al servidor de handlers
	...
	puerto := obtenerEnv("APP_PORT", "8080")
	fmt.Printf("Servidor de Archibase corriendo en http://localhost:%s\n", puerto)
	if err := http.ListenAndServe(":"+puerto, enrutador); err != nil {
